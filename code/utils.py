...
def PSNR(orimg, estimg, pattern):
    ...
    :returns PSNR: tuple contains PSNR of R, G, B channels and the whole image.
    ...
    for i in range(3):
        ...
        mse = np.linalg.norm((1 - mask[:,:,i])*diff)**2
        nitem = (1-mask[:,:,i]).sum()
        PSNR[i] = 10*np.log10(255**2/(mse/nitem))
        total_mse += mse 
        num_items += nitem
    
    PSNR[3] = 10*np.log10(255**2/(total_mse/num_items))
